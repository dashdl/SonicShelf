...
const getUserList = async () => {
  ...
  if (res.code === '200') {
    // 适配真实数据结构：将时间字段统一转换为驼峰命名并确保 status 为数字
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除该用户吗？', '提示', {
    ...
    try {
      const res = await request.delete(`users/delete/${row.id}`)
      if (res.code === '200') {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    let res
    if (userForm.id) {
      res = await request.post('users/update', userForm)
    } else {
      res = await request.post('users/add', userForm)
    }
    if (res.code === '200') {
...
const handleStatusChange = async (row) => {
  try {
    const res = await request.post('users/updateStatus', {}, {
      params: {
        id: row.id,
        status: row.status
      }
